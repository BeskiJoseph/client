...
export default function Alldata() {
    ...
    async function handleClick(e) {
        e.preventDefault();
        try {
            let result = await axios.get("https://server-sk7t.onrender.com/data");
            setData(result.data);
        } catch (error) {
    ...
    async function handleDelete(userId) {
        try {
            let response = await axios.delete(`https://server-sk7t.onrender.com/delete/${userId}`);
    ...
    async function handleUpdate(e) {
        e.preventDefault();
        try {
            let response = await axios.put(`https://server-sk7t.onrender.com/update/${editUser.email}`, updatedUser);
            if (response.data.success) {
                    ...
                    {data.map((item) => (
                        <tr key={item.email}>
                            ...
                            <td className="action-buttons">
                                <button className="blue-button" onClick={() => handleDelete(item._id)}>Delete</button>
                                <button className="blue-button" onClick={() => handleEdit(item)}>Edit</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
